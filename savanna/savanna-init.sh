...
sed -i "s/MASTER_ID/$MASTER_ID/g" cluster.tmp
sed -i "s/WORKER_ID/$WORKER_ID/g" cluster.tmp
...
export IMAGE_ID=$(nova --os-username=admin --os-tenant=admin --os-password=admin --os-auth-url=$AUTH_URL image-list | grep fedora-hdp.image | awk {'print $2'})
http http://localhost:18080/v1.0/$TENANT/images/$IMAGE_ID X-Auth-Token:$TOKEN < image.reg
http http://localhost:18080/v1.0/$TENANT/images/$IMAGE_ID/tag X-Auth-Token:$TOKEN < tags.reg
